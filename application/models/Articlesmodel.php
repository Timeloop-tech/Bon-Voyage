...
class Articlesmodel extends CI_Model{
    public function articles_list($limit,$offset){
        ...
        $query = $this->db->select(['title','id','image_path','body'])
                             ->from('articles')
    ...
    public function all_articles_list($limit,$offset){
        $query = $this->db->select(['title','id','created_at','image_path','user_id'])
            ->from('articles')
            ->limit($limit,$offset)
    ...
    public function find_article($article_id){
        $q = $this->db->select(['id','user_id','title','body','image_path'])
                        ->where('id',$article_id)
                        ->get('articles');
        return $q->row();
    }
